...
loan <- read.csv(file="train_u6lujuX_CVtuZ9i.csv",stringsAsFactors = F)
data.test <- read.csv(file="test_Y3wMUE5_7gLdaTN.csv",stringsAsFactors = F)
dim(loan)
str(loan)
summary(loan)
View(loan)

#Variable Identificationa and Conversion
...
#Univariate Analysis
colnames(loan)
plot(loan$ApplicantIncome)
hist(loan$ApplicantIncome)
plot(loan$CoapplicantIncome)
plot(loan$LoanAmount)
hist(loan$LoanAmount)
plot(loan$Loan_Amount_Term)
...
#Imputing values in variables where there are no values
...
#Imputing Missing Values for Credit_History using ctree
...
loan$D_LoanStatus_Yes <- ifelse(loan$Loan_Status=="Y" ,1,0)

colnames(loan)

#PREDICITNG vALUES FOR TEST DATA
...
install.packages(c("randomForest","mlbench","caret","e1071"))
...
# Load Dataset
# Followed article to improve accuracy using random forest
...
write.csv(Results,"Final_Result6.csv",row.names=FALSE)
